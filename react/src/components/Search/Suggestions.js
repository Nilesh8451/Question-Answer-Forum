  ...
  render() {
    ...
    if (this.props.results.length > 0) {
      options = this.props.results.map(r => {
        ...
        if (this.props.allinfo.length > 0) {
          op = this.props.allinfo.map(rout => {
            if (rout.title === r) {
        ...
        return (<div className='burger'>
          <NavLink to={"/Ques/" + (lucky)}> <span key={this.props.results.indexOf(r)} onClick={() => this.showques(r)} style={{color:'black'}}>
            {r}
          </span>
          </NavLink>
        </div>
