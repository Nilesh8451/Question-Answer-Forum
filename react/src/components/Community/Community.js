...
class Community extends Component {
    ...
    componentWillMount() {
        this.Auth.fetch(this.state.apiurl+'viewcommunity', {
        ...
        this.Auth.fetch(this.state.apiurl+'viewuser', {
            method: 'POST', body: JSON.stringify({
    ...
    addProfileDPHandler = () => {
        ...
        this.Auth.ifetch(this.state.apiurl+'communitydp', {
            method: 'POST', body: formData,


        }).then(res => {
            // console.log('Edited Community Photo successfully')
        })
            .catch(err => {
                // console.log('Community Image error', err)
            })
    ...
    joincommu = (commuid) => {
        ...
        this.Auth.fetch(this.state.apiurl+'follow', {
            ...
            .catch(err => {
                // console.log('Not joined sorry error', err)
            })
    }
    ...
    render() {
        ...
        let cd = this.state.com.map(lat => {
            if (lat.Name === this.state.qid) {
                return (
                    <div className="Cdeveloperstory">
                        <div className="CDSIMAGEDIV">
                            <img src={pro} />
                        </div>
                        <div className="CDSNAMEDIV">
                            <h3>{lat.Name}</h3>
                        </div>
                        <div className="CDSDiscription">
                            <p>{lat.description}</p>
                        </div>
                        <div className="CDSinfodisplay">
