    ...
    const r = User.findOne({ userName: username }).then((resq) => {
        ...
        if (!resq.upvotedques.includes(req.body.qid) && !resq.downvotedques.includes(req.body.qid)) {
            ...
            const results = Question.findOneAndUpdate({ _id: req.body.qid }, { $inc: { upvotes: 1 } }).then((updatedDoc) => {
                ...
                const result1 = User.findOneAndUpdate({ userName: updatedDoc.creatorname }, { $inc: { rep: 5 } }).then((updated1Doc) => { //console.log(updated1Doc);
        ...
        if (resq.upvotedques.includes(req.body.qid) && !resq.downvotedques.includes(req.body.qid)) {
            // console.log("2")
            const results = Question.findOneAndUpdate({ _id: req.body.qid }, { $inc: { upvotes: -1 } }).then((updatedDoc) => {  //console.log(updatedDoc.creatorname);
                const result1 = User.findOneAndUpdate({ userName: updatedDoc.creatorname }, { $inc: { rep: -5 } }).then((updated1Doc) => {// console.log(updated1Doc);
