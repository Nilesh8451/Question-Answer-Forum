...
router.post('/', (req, res) => {
    ...
    const results = Answer.findOneAndUpdate({ _id: req.body.aid, askedby: username }, { accepted: true }).then((updatedDoc) => { //console.log(update);
        const r = Question.findOneAndUpdate({ _id: updatedDoc.qid }, { accepted: req.body.aid }).then((re => {
            // console.log(re);
        }))

        const result1 = User.findOneAndUpdate({ userName: updatedDoc.creatorname }, { $inc: { rep: 15 } }).then((updated1Doc) => { //console.log(updated1Doc);
